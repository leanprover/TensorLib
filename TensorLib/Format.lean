...
private def toList (a : Type) [TensorElement a] (x : NumpyRepr) : Err (List a) :=
  let traverseFn ind : Err a := Index.getDimIndex x ind
  traverse traverseFn x.shape.allDimIndices

private def toList! (a : Type) [TensorElement a] (x : NumpyRepr) : List a := match toList a x with
| .error _ => []
| .ok x => x

#guard toList! BV16 (TensorElement.arange BV16 10) == [0x0000#16, 0x0001#16, 0x0002#16, 0x0003#16, 0x0004#16, 0x0005#16, 0x0006#16, 0x0007#16, 0x0008#16, 0x0009#16]
...
private def toTree {a : Type} (x : List a) (strides : Strides) : Err (Tree a) := match strides with
| [] => if x.length == 1 then .ok (.root x) else .error "empty shape that's not an array scalar"
| [1] => .ok (.root x)
| [_] => .error "not a unit stride"
| stride :: strides => do
  let chunks := x.toChunks stride
...
#eval toTree! (toList! BV16 (TensorElement.arange BV16 10)) [5, 1]
...
def format (a : Type) [Repr a] [TensorElement a] (x : NumpyRepr) : Err Std.Format := do
  let xs <- toList a x
  let t <- toTree xs x.unitStrides
  let f <- formatTree t x.shape
  return f

def reprToString (a : Type) [Repr a] [TensorElement a] (x :  NumpyRepr) : String := match format a x with
| .error err => s!"Error: {err}"
| .ok s => pretty s 120

#guard reprToString BV8 (TensorElement.arrayScalar (5 : BV8)) == "array(0x05#8)"
#guard reprToString BV8 (TensorElement.arange BV8 10) == "array([0x00#8, 0x01#8, 0x02#8, 0x03#8, 0x04#8, 0x05#8, 0x06#8, 0x07#8, 0x08#8, 0x09#8])"
