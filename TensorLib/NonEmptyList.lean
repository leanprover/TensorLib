...
theorem appendListLLength (x : List a) (y : NonEmptyList a) : (appendListL x y).length = x.length + y.length := by
  induction x
  . simp
  . simp [NonEmptyList.length, NonEmptyList.toList]
    linarith

instance HAppendListL : HAppend (List a) (NonEmptyList a) (NonEmptyList a) where
  hAppend := appendListL

theorem hAppendListLLength (x : List a) (y : NonEmptyList a) : (x ++ y).length = x.length + y.length := by
...
theorem hAppendListRLength (x : NonEmptyList a) (y : List a) : (x ++ y).length = x.length + y.length := by
