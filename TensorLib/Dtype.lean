...
-- private def castEndian (fromDtype toDtype : Dtype) (data : Array UInt8) : Err (Array UInt8) :=
--   if fromDtype == toDtype then .ok data else
--   if fromDtype.name != toDtype.name then .error "different dtype names" else
--   match fromDtype.order, toDtype.order with
--   | .littleEndian, .bigEndian
--   | .bigEndian, .littleEndian => .ok data.reverse
--   | _, _ => .error "Illegal dtype"

private def leftExtend0 (arr : Array UInt8) (size : Nat) : Array UInt8 :=
  if size <= arr.size then arr else
  let diff := size - arr.size
  let pre := (List.replicate diff (0 : UInt8)).toArray
  pre ++ arr

  -- let orderArray (order : ByteOrder) (arr : Array UInt8) : Err (Array UInt8) := match order with
  -- | .littleEndian => .ok arr.reverse
  -- | .bigEndian => .ok arr
  -- | .oneByte => if arr.size == 1 then .ok arr else .error "bad block size"
  -- Assume arrays are big endian, and reverse them for littleendian
    -- let intValue (pre : Array UInt8) : Err (Array UInt8) :=
    --   if b0 <= 0x7F then .ok $ Array.append pre #[b0] else .error "uint8 out of bounds for int8"
...
private def cast1LittleEndian (fromDtype toDtype : Dtype) (data : Array UInt8) : Err (Array UInt8) :=
  let fromOrder := fromDtype.order
  if fromOrder != .littleEndian && fromOrder != .oneByte then .error "Expected littleEndian or oneByte order" else
  let toOrder := toDtype.order
  if toOrder != .littleEndian && toOrder != .oneByte then .error "Expected littleEndian or oneByte order" else
  if fromDtype == toDtype then .ok data else
  let fromBlockSize := fromDtype.itemsize
  if data.size != fromBlockSize then .error "bad block size" else -- could optimize away this check
  match fromDtype.name, data, toDtype.name with
  -- bool
  | .bool, #[b0], .uint8 => .ok #[b0]
  | .bool, #[b0], .uint16 => .ok #[b0, 0]
  | .bool, #[b0], .uint32 => .ok #[b0, 0, 0, 0]
  | .bool, #[b0], .uint64 => .ok #[b0, 0, 0, 0, 0, 0, 0, 0]
  | .bool, #[b0], .int8 => .ok #[b0]
  | .bool, #[b0], .int16 => .ok #[b0, 0]
  | .bool, #[b0], .int32 => .ok #[b0, 0, 0, 0]
  | .bool, #[b0], .int64 => .ok #[b0, 0, 0, 0, 0, 0, 0, 0]
  | .bool, #[b0], .float64 => .ok $ byteToFloatArray b0
  -- uint8
  | .uint8, #[b0], .bool => if b0 <= 1 then .ok #[b0] else .error "overflow"
  | .uint8, #[b0], .uint16 => .ok #[b0, 0]
  | .uint8, #[b0], .uint32 => .ok #[b0, 0, 0, 0]
  | .uint8, #[b0], .uint64 => .ok #[b0, 0, 0, 0, 0, 0, 0, 0]
  | .uint8, #[b0], .int8 => if b0 <= 0x7F then .ok #[b0] else .error "underflow"
  | .uint8, #[b0], .int16 => .ok #[b0, 0]
  | .uint8, #[b0], .int32 => .ok #[b0, 0, 0, 0]
  | .uint8, #[b0], .int64 => .ok #[b0, 0, 0, 0, 0, 0, 0, 0]
  | .uint8, #[b0], .float64 => .ok $ byteToFloatArray b0
  -- uint16
  | .uint16, #[b0, b1], .bool => if b1 == 0 && b0 <= 1 then .ok #[b0, 0] else .error "overflow"
  | .uint16, #[b0, b1], .uint8 => if b1 == 0 then .ok #[b0] else .error "overflow"
  | .uint16, #[b0, b1], .uint32 => .ok #[b0, b1, 0, 0]
  | .uint16, #[b0, b1], .uint64 => .ok #[b0, b1, 0, 0, 0, 0, 0, 0]
  | .uint16, #[b0, b1], .int8 => if b1 == 0 && b0 <= 0x7F then .ok #[b0] else .error "overflow"
  | .uint16, #[b0, b1], .int16 => if b1 <= 0x7F then .ok #[b0, b1] else .error "underflow"
  | .uint16, #[b0, b1], .int32 => .ok #[b0, b1, 0, 0]
  | .uint16, #[b0, b1], .int64 => .ok #[b0, b1, 0, 0, 0, 0, 0, 0]
  | .uint16, #[_, _], .float64 => natToFloatByteArray (unsignedLEByteArrayToNat data)
  -- uint32
  | .uint32, #[b0, b1, b2, b3], .bool => if b3 == 0 && b2 == 0 && b1 == 0 && b0 <= 1 then .ok #[b0] else .error "overflow"
  | .uint32, #[b0, b1, b2, b3], .uint8 => if b3 == 0 && b2 == 0 && b1 == 0 then .ok #[b0] else .error "overflow"
  | .uint32, #[b0, b1, b2, b3], .uint16 => if b3 == 0 && b2 == 0 then .ok #[b0, b1] else .error "overflow"
  | .uint32, #[b0, b1, b2, b3], .uint64 => .ok #[b0, b1, b2, b3, 0, 0, 0, 0]
  | .uint32, #[b0, b1, b2, b3], .int8 => if b3 == 0 && b2 == 0 && b1 == 0 && b0 <= 0x7F then .ok #[b0] else .error "overflow"
  | .uint32, #[b0, b1, b2, b3], .int16 => if b3 == 0 && b2 == 0 && b1 <= 0x7F then .ok #[b0, b1] else .error "underflow"
  | .uint32, #[b0, b1, b2, b3], .int32 => if b3 <= 0x7F then .ok #[b0, b1, b2, b3] else .error "underflow"
  | .uint32, #[b0, b1, b2, b3], .int64 => .ok #[b0, b1, b2, b3, 0, 0, 0, 0]
  | .uint32, #[_, _, _, _], .float64 => natToFloatByteArray (unsignedLEByteArrayToNat data)
  -- uint64
  | .uint64, #[b0, b1, b2, b3, b4, b5, b6, b7], .bool => if b7 == 0 && b6 == 0 && b5 == 0 && b4 == 0 && b3 == 0 && b2 == 0 && b1 == 0 && b0 <= 1 then .ok #[b0] else .error "overflow"
  | .uint64, #[b0, b1, b2, b3, b4, b5, b6, b7], .uint8 => if b7 == 0 && b6 == 0 && b5 == 0 && b4 == 0 && b3 == 0 && b2 == 0 && b1 == 0 then .ok #[b0] else .error "overflow"
  | .uint64, #[b0, b1, b2, b3, b4, b5, b6, b7], .uint16 => if b7 == 0 && b6 == 0 && b5 == 0 && b4 == 0 && b3 == 0 && b2 == 0 then .ok #[b0, b1] else .error "overflow"
  | .uint64, #[b0, b1, b2, b3, b4, b5, b6, b7], .uint32 => if b7 == 0 && b6 == 0 && b5 == 0 && b4 == 0 then .ok #[b0, b1, b2, b3, 0, 0, 0, 0] else .error "overflow"
  | .uint64, #[b0, b1, b2, b3, b4, b5, b6, b7], .int8 => if b7 == 0 && b6 == 0 && b5 == 0 && b4 == 0 && b3 == 0 && b2 == 0 && b1 == 0 && b0 <= 0x7F then .ok #[b0] else .error "overflow"
  | .uint64, #[b0, b1, b2, b3, b4, b5, b6, b7], .int16 => if b7 == 0 && b6 == 0 && b5 == 0 && b4 == 0 && b3 == 0 && b2 == 0 && b1 <= 0x7F then .ok #[b0, b1] else .error "underflow"
  | .uint64, #[b0, b1, b2, b3, b4, b5, b6, b7], .int32 => if b7 == 0 && b6 == 0 && b5 == 0 && b4 == 0 && b3 <= 0x7F then .ok #[b0, b1, b2, b3] else .error "underflow"
  | .uint64, #[b0, b1, b2, b3, b4, b5, b6, b7], .int64 => if b7 <= 0x7F then .ok #[b0, b1, b2, b3, b4, b5, b6, b7] else .error "underflow"
  | .uint64, #[_, _, _, _, _, _, _, _], .float64 => natToFloatByteArray (unsignedLEByteArrayToNat data)
  | .int8, _, _
  | .int16, _, _
  | .int32, _, _
  | .int64, _, _
  | .float64, _, _ => .error "Unimplemented: just needs work"
  | .float16, _, _
  | _, _, .float16
  | .float32, _, _
  | _, _, .float32 => .error "Unimplemented: needs Lean support of sub-word float types"
  | _, _, _ => .error "Data/dtype mismatch"

private def cast1 (fromDtype toDtype : Dtype) (data : Array UInt8) : Err (Array UInt8) :=
  match fromDtype.order, toDtype.order with
  | .littleEndian, .littleEndian
  | .oneByte, .littleEndian
  | .littleEndian, .oneByte
  | .oneByte, .oneByte => cast1LittleEndian fromDtype toDtype data
  | .bigEndian, .littleEndian
  | .bigEndian, .oneByte => cast1LittleEndian { fromDtype with order := .littleEndian } toDtype data.reverse
  | .bigEndian, .bigEndian => (cast1LittleEndian { fromDtype with order := .littleEndian } toDtype data.reverse).map fun arr => arr.reverse
  | .littleEndian, .bigEndian
  | .oneByte, .bigEndian => (cast1LittleEndian fromDtype toDtype data).map fun arr => arr.reverse

def cast (fromDtype toDtype : Dtype) (data : ByteArray) : Err ByteArray := do
  let fromBlockSize := fromDtype.itemsize
  if data.size % fromBlockSize != 0 then .error "Input data not a multiple of the item size" else
  let numBlocks := data.size / fromDtype.itemsize
  let mut res := #[]
  for i in [0:numBlocks] do
    let fromBlock := data.copySlice (i + fromBlockSize) ByteArray.empty 0 fromBlockSize
    let toBlock <- cast1 fromDtype toDtype fromBlock.data
    res := res.append toBlock
  return ByteArray.mk res
