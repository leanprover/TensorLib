...
def assign (a : Type) [typ : Tensor.Element a] (arr : Tensor) (index : NumpyBasic) (v : Tensor) : Err Tensor := do
  ...
  match Broadcast.broadcast (Broadcast.mk shape v.shape) with
  | .none => .error "Can't broadcast shapes"
  | .some vShape =>
  let v <- v.reshape vShape
  let aIter := BasicIter.make basic
  let mut vIter := DimsIter.make shape
  let mut res := arr
  for aIndex in aIter do
    let vIndexIter := vIter.next
    let vIndex := vIndexIter.1
    vIter := vIndexIter.2
